...
# Script to import validator anti-slashing database from EIP-3076 format.
#
# This script routes to the appropriate VC-specific import script based on the VC environment variable.
#
# Usage: VC=vc-lodestar ./scripts/edit/vc/import_asdb.sh [options]
#
...
VC_SCRIPT="${SCRIPT_DIR}/${VC_NAME}/import_asdb.sh"

# Check if the VC-specific script exists
if [ ! -f "$VC_SCRIPT" ]; then
    echo "Error: Import script for '$VC' not found at: $VC_SCRIPT" >&2
    echo "" >&2
    echo "Available VC types:" >&2
    for dir in "${SCRIPT_DIR}"/*; do
        if [ -d "$dir" ] && [ -f "$dir/import_asdb.sh" ]; then
            basename "$dir"
        fi
    done | sed 's/^/  - vc-/' >&2
    exit 1
