...
# Integration test for recreate-private-keys.sh script
#
# This test validates:
...
#
# Usage: ./scripts/edit/recreate-private-keys/test/test_recreate_private_keys.sh
...
# Script under test
RECREATE_SCRIPT="$REPO_ROOT/scripts/edit/recreate-private-keys/recreate-private-keys.sh"
...
case "$*" in
    ...
    *"charon"*"enr"*)
        ...
        exit 0
        ;;
    *"charon"*"edit recreate-private-keys"*)
        echo "[MOCK] Running recreate-private-keys"
        exit 0
        ;;
    *)
...
# ============================================================================
# RECREATE-PRIVATE-KEYS.SH TESTS
# ============================================================================
...
test_help() {
    ...
    output=$("$RECREATE_SCRIPT" --help 2>&1) || exit_code=$?
...
test_missing_env() {
    ...
    output=$("$RECREATE_SCRIPT" 2>&1) || exit_code=$?
...
test_missing_validator_keys() {
    local output
    local exit_code=0
    
    # Ensure validator_keys doesn't exist
    rm -rf "$REPO_ROOT/.charon/validator_keys"
    
    output=$("$RECREATE_SCRIPT" 2>&1) || exit_code=$?
    
    assert_exit_code 1 "$exit_code" "test_missing_validator_keys" && \
    assert_output_contains "validator_keys directory not found" "$output" "test_missing_validator_keys"
}
...
test_dry_run_workflow() {
    ...
    # Create validator_keys for this test
    mkdir -p "$REPO_ROOT/.charon/validator_keys"
    
    output=$("$RECREATE_SCRIPT" --dry-run 2>&1) || exit_code=$?
    
    # Cleanup
    rm -rf "$REPO_ROOT/.charon/validator_keys"
    
    assert_exit_code 0 "$exit_code" "test_dry_run_workflow" && \
    assert_output_contains "DRY-RUN MODE" "$output" "test_dry_run_workflow" && \
    assert_output_contains "Exporting anti-slashing database" "$output" "test_dry_run_workflow" && \
    assert_output_contains "charon alpha edit recreate-private-keys" "$output" "test_dry_run_workflow" && \
    assert_output_contains "Updating anti-slashing database" "$output" "test_dry_run_workflow" && \
    ...
    assert_output_contains "Importing updated anti-slashing" "$output" "test_dry_run_workflow" && \
...
test_unknown_argument() {
    ...
    output=$("$RECREATE_SCRIPT" --invalid-flag 2>&1) || exit_code=$?
...
main() {
    echo ""
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║     Recreate-Private-Keys Script - Integration Tests           ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo ""
    echo "─────────────────────────────────────────────────────────────────"
    echo " RECREATE-PRIVATE-KEYS.SH TESTS"
    echo "─────────────────────────────────────────────────────────────────"
    echo ""
    
    run_test "recreate-private-keys: --help shows usage" test_help
    run_test "recreate-private-keys: error when .env missing" test_missing_env
    run_test "recreate-private-keys: error when NETWORK missing" test_missing_network
    run_test "recreate-private-keys: error when VC missing" test_missing_vc
    run_test "recreate-private-keys: error when .charon dir missing" test_missing_charon_dir
    run_test "recreate-private-keys: error when cluster-lock missing" test_missing_cluster_lock
    run_test "recreate-private-keys: error when validator_keys missing" test_missing_validator_keys
    run_test "recreate-private-keys: dry-run full workflow" test_dry_run_workflow
    run_test "recreate-private-keys: error for unknown argument" test_unknown_argument
