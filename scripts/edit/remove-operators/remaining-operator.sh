...
# Remove-Operators Script for REMAINING Operators
#
# This script automates the remove-operators ceremony for operators who are
# staying in the cluster. It handles the full workflow including ASDB
...
#
# IMPORTANT: This is a CEREMONY - ALL participating operators must run their
# respective scripts simultaneously. The ceremony coordinates between
# participants to generate new key shares for the reduced operator set.
#
# The workflow:
# 1. Export the current anti-slashing database
# 2. Run the remove-operators ceremony (all participating operators simultaneously)
...
#
# Usage:
#   ./scripts/edit/remove-operators/remaining-operator.sh [OPTIONS]
#
# Options:
