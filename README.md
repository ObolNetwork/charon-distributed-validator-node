...
docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v0.10.1 create enr
```
...
docker run --rm -v "$(pwd):/opt/charon" --env-file .env.create_dkg obolnetwork/charon:v0.10.1 create dkg
```
...
docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v0.10.1 dkg --p2p-bootnode-relay
```
...
2. How do I get my ENR if I want to generate it again?
   ...
   - Run `docker run --rm -v "$(pwd):/opt/charon" obolnetwork/charon:v0.10.1 enr`. This prints the ENR on your screen.
...
6. How do I override the default config provided in this repo with my own custom values?
   ...
   - This functionality is made possible by the [special env var syntax](https://docs.docker.com/compose/environment-variables/#substitute-environment-variables-in-compose-files) in `docker-compose.yml` which defines a default if the env var isn't present. E.g., `${CHARON_VERSION:-v0.10.1}` defaults to `v0.10.1` or to the value of `CHARON_VERSION` env var if present.
